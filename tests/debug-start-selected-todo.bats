...
save_snapshot() {
  (cd "$TEST_TEMP_DIR" && LBWC_PLANNING_DIR="$LBWC_PLANNING_DIR" bash "$LIFECYCLE_SCRIPT" list-with-snapshot) >/dev/null
}
...
active_session_count() {
  find "$LBWC_PLANNING_DIR/debugging/active" -maxdepth 1 -type f -name '*.md' 2>/dev/null | wc -l | tr -d ' '
}

completed_session_count() {
  find "$LBWC_PLANNING_DIR/debugging/completed" -maxdepth 1 -type f -name '*.md' 2>/dev/null | wc -l | tr -d ' '
}

pending_todos_contains() {
  local needle="$1"
  awk '/^## Todos$/{found=1; next} found && /^## /{exit} found {print}' "$LBWC_PLANNING_DIR/STATE.md" | grep -qF "$needle"
}
...
@test "debug-start-selected-todo: no-ref happy path creates session and removes root todo" {
  ...
  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" 1
  [ "$status" -eq 0 ]
  ...
  grep -q 'Picked up todo via /lbwc:debug: Fix parser bug' "$LBWC_PLANNING_DIR/STATE.md"
...
@test "debug-start-selected-todo: ref detail success persists detail signal and safely removes sidecar" {
  write_state
  bash "$DETAILS_SCRIPT" add deadbeef '{"summary":"Investigate detail-backed crash","context":"Crash detail context","files":["src/parser.sh","tests/parser.bats"],"added":"2026-04-02","source":"session"}' "$LBWC_PLANNING_DIR/todo-details.json" >/dev/null
  save_snapshot

  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" 2
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "ok" ]
  ...
  ! grep -q 'Investigate detail-backed crash' "$LBWC_PLANNING_DIR/STATE.md"
...
@test "debug-start-selected-todo: structurally empty ok detail remains sparse but still cleans root todo" {
  write_state
  bash "$DETAILS_SCRIPT" add feedcafe '{"summary":"Empty detail item","context":"","files":[],"added":"2026-04-03","source":"session"}' "$LBWC_PLANNING_DIR/todo-details.json" >/dev/null
  save_snapshot

  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" 3
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "ok" ]
  [ "$(printf '%s' "$output" | jq -r '.detail_status')" = "ok" ]
...
@test "debug-start-selected-todo: missing detail warns, creates session, and removes root todo" {
  ...
  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" 4
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "ok" ]
  ...
  ! grep -q 'Missing detail item' "$LBWC_PLANNING_DIR/STATE.md"
  grep -q 'Detail for ref f78562c0 could not be loaded' "$LBWC_PLANNING_DIR/STATE.md"
}
...
EOF
  ...
  eval "$(printf '%s' "$source_json" | bash "$SCRIPTS_DIR/debug-session-state.sh" start-with-source-todo "$LBWC_PLANNING_DIR" already-completed-bug)"
...
EOF
  ...
  eval "$(printf '%s' "$source_json" | bash "$SCRIPTS_DIR/debug-session-state.sh" start-with-source-todo "$LBWC_PLANNING_DIR" legacy-completed-bug)"
  bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" complete >/dev/null
  legacy_file="$LBWC_PLANNING_DIR/debugging/${session_id}.md"
  mv "$LBWC_PLANNING_DIR/debugging/completed/${session_id}.md" "$legacy_file"
  rmdir "$LBWC_PLANNING_DIR/debugging/completed"
  [ -f "$legacy_file" ]
  save_snapshot
...
EOF
  ...
  eval "$(printf '%s' "$source_json" | bash "$SCRIPTS_DIR/debug-session-state.sh" start-with-source-todo "$LBWC_PLANNING_DIR" duplicate-bug)"
  bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" complete >/dev/null
  save_snapshot
...
EOF
  ...
  cat > "$LBWC_PLANNING_DIR/debugging/completed/20260401-010101-old-one.md" <<'EOF'
---
...
- **Detail Status:** none
EOF
  cat > "$LBWC_PLANNING_DIR/debugging/completed/20260401-020202-old-two.md" <<'EOF'
---
...
EOF
  ...
  run bash "$instrumented_dir/debug-start-selected-todo.sh" "$LBWC_PLANNING_DIR" 1
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "ok" ]
  [ "$(active_session_count)" = "1" ]
...
@test "debug-start-selected-todo: resolver errors do not create sessions or mutate state" {
  ...
  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" 99
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "error" ]
  [ "$(printf '%s' "$output" | jq -r '.code')" = "invalid_selection" ]
  [ ! -d "$LBWC_PLANNING_DIR/debugging/active" ]
  grep -q 'Fix parser bug' "$LBWC_PLANNING_DIR/STATE.md"
}

@test "debug-start-selected-todo: root-state rejection leaves archived state untouched" {
  rm -f "$LBWC_PLANNING_DIR/STATE.md"
  mkdir -p "$LBWC_PLANNING_DIR/milestones/old"
  cat > "$LBWC_PLANNING_DIR/milestones/old/STATE.md" <<'EOF'
# Project State
...
EOF
  ...
  [ "$(printf '%s' "$output" | jq -r '.status')" = "error" ]
  [ ! -d "$LBWC_PLANNING_DIR/debugging/active" ]
  grep -q 'Archived bug' "$LBWC_PLANNING_DIR/milestones/old/STATE.md"
}
...
@test "debug-start-selected-todo: absolute planning-dir isolation works from another cwd" {
  ...
  local target_planning="$target_root/.lbwc-planning"
  local other_planning="$other_root/.lbwc-planning"
...
EOF
  ...
  (cd "$target_root" && LBWC_PLANNING_DIR="$target_planning" bash "$LIFECYCLE_SCRIPT" list-with-snapshot) >/dev/null
...
@test "debug-start-selected-todo: routing flags are preserved" {
  ...
  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" 1 --parallel --serial
  [ "$status" -eq 0 ]
...
- 2026-04-01: Existing note
EOF
  bash "$DETAILS_SCRIPT" add decafbad '{"summary":"Marker bug","context":"Accepted UAT summary deviation","files":[],"added":"2026-04-01","source":"uat-deviation","uat_deviation":{"id":"D1"},"known_issue_signature":{"disposition":"accepted-process-exception"}}' "$LBWC_PLANNING_DIR/todo-details.json" >/dev/null
  save_snapshot
...
@test "debug-start-selected-todo: usage errors are compact parseable JSON" {
  run bash "$SCRIPT" "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "error" ]
  [ "$(printf '%s' "$output" | jq -r '.code')" = "usage" ]

  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" not-a-number
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "error" ]
  [ "$(printf '%s' "$output" | jq -r '.code')" = "usage" ]

  run bash "$SCRIPT" "$LBWC_PLANNING_DIR" 1 --unknown
  [ "$status" -eq 0 ]
  [ "$(printf '%s' "$output" | jq -r '.status')" = "error" ]
  [ "$(printf '%s' "$output" | jq -r '.code')" = "usage" ]
}
