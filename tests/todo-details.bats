...
setup() {
  setup_temp_dir
  create_test_config
  DETAILS_PATH="$TEST_TEMP_DIR/.lbwc-planning/todo-details.json"
}
...
@test "add writes only canonical registry and does not create legacy fallback file" {
  cd "$TEST_TEMP_DIR"
  rm -f "$DETAILS_PATH"
  rm -rf "$TEST_TEMP_DIR/.lbwc-planning/todo-details"
  ...
  [ -f "$DETAILS_PATH" ]
  [ ! -f "$TEST_TEMP_DIR/.lbwc-planning/todo-details/feedcafe.json" ]
}
...
@test "get falls back to legacy per-file detail when canonical registry entry is missing" {
  cd "$TEST_TEMP_DIR"
  rm -f "$DETAILS_PATH"
  mkdir -p "$TEST_TEMP_DIR/.lbwc-planning/todo-details"
  cat > "$TEST_TEMP_DIR/.lbwc-planning/todo-details/a1b2c3d4.json" <<'EOF'
...
@test "remove deletes legacy fallback alongside canonical registry entry" {
  ...
  mkdir -p "$TEST_TEMP_DIR/.lbwc-planning/todo-details"
  cat > "$TEST_TEMP_DIR/.lbwc-planning/todo-details/deadbeef.json" <<'EOF'
...
EOF
  ...
  [ ! -f "$TEST_TEMP_DIR/.lbwc-planning/todo-details/deadbeef.json" ]
...
@test "gc removes orphaned entries not in STATE.md" {
  cd "$TEST_TEMP_DIR"
  local state_path="$TEST_TEMP_DIR/.lbwc-planning/STATE.md"
  cat > "$state_path" <<'EOF'
# State
## Todos
