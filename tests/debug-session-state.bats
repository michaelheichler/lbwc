...
@test "start creates session file and active pointer" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "test-bug"
  [ "$status" -eq 0 ]
  ...
  [ -f "$LBWC_PLANNING_DIR/debugging/.active-session" ]
...
@test "start sanitizes slug" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "My Bug Has Spaces & Symbols!"
  [ "$status" -eq 0 ]
  eval "$output"
...
@test "start fails without slug" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR"
  [ "$status" -eq 1 ]
}
...
@test "start-with-source-todo rolls back the new session on writer failure" {
  ...
  [ "$status" -eq 1 ]
  [ ! -f "$LBWC_PLANNING_DIR/debugging/.active-session" ]
  run find "$LBWC_PLANNING_DIR/debugging/active" -maxdepth 1 -type f -name '*.md'
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}

@test "start-with-source-todo restores the prior active pointer on writer failure" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "first-bug")"
  ...
  [ "$status" -eq 1 ]
  [ -f "$LBWC_PLANNING_DIR/debugging/.active-session" ]
  [ "$(cat "$LBWC_PLANNING_DIR/debugging/.active-session")" = "$first_session_name" ]
  [ -f "$LBWC_PLANNING_DIR/debugging/active/$first_session_name" ]
}
...
@test "get returns none when no active session" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" get "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
...
@test "get returns session metadata when active" {
  ...
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "get-test")"
...
@test "get-or-latest returns none when no sessions exist" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" get-or-latest "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
  [[ "$output" == *"active_session=none"* ]]
}
...
@test "get-or-latest falls back to latest unresolved" {
  ...
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "fallback-test")"
  rm -f "$LBWC_PLANNING_DIR/debugging/.active-session"
...
@test "get-or-latest metadata eval succeeds when status shell variable is readonly" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "readonly-fallback")"
  rm -f "$LBWC_PLANNING_DIR/debugging/.active-session"
  ...
  ' -- "$SCRIPTS_DIR/debug-session-state.sh" "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
  [[ "$output" == *"active_session=fallback"* ]]
  [[ "$output" == *"session_status=investigating"* ]]
  [[ "$output" == *"session_file=$LBWC_PLANNING_DIR/debugging/active/"* ]]
}

@test "get-or-latest skips completed sessions in fallback" {
  # Create two sessions, complete the first
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "old-bug")"
  local old_file="$session_file"
  bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" complete > /dev/null

  # Wait 1 second to ensure different timestamp
  sleep 1

  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "new-bug")"
...
@test "resume sets active pointer to specified session" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "resume-test")"
  local sid="$session_id"
  bash "$SCRIPTS_DIR/debug-session-state.sh" clear-active "$LBWC_PLANNING_DIR" > /dev/null

  run bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "$sid"
  [ "$status" -eq 0 ]
  [[ "$output" == *"active_session=true"* ]]
  printf '%s\n' "$output" | grep -Eq '^session_status=investigating$'
  ! printf '%s\n' "$output" | grep -Eq '^status='
...
@test "resume metadata eval succeeds when status shell variable is readonly" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "readonly-resume")"
  local sid="$session_id"
  bash "$SCRIPTS_DIR/debug-session-state.sh" clear-active "$LBWC_PLANNING_DIR" > /dev/null
  ...
  ' -- "$SCRIPTS_DIR/debug-session-state.sh" "$LBWC_PLANNING_DIR" "$sid"
  [ "$status" -eq 0 ]
  [[ "$output" == *"active_session=true"* ]]
  [[ "$output" == *"session_status=investigating"* ]]
...
@test "resume fails for nonexistent session" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "20990101-000000-no-such-session.md"
  [ "$status" -eq 1 ]
  [[ "$output" == *"not found"* ]]
}
...
@test "resume rejects malformed session names" {
  # Path traversal
  run bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "../etc/passwd"
  [ "$status" -eq 1 ]

  # No timestamp prefix
  run bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "nonexistent"
  [ "$status" -eq 1 ]
  [[ "$output" == *"YYYYMMDD-HHMMSS-slug"* ]]

  # Wrong extension
  run bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "20990101-000000-foo.txt"
  [ "$status" -eq 1 ]
}
...
@test "set-status transitions through valid states" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "status-test")"
  ...
  for s in fix_applied qa_pending qa_failed uat_pending uat_failed; do
    run bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" "$s"
    [ "$status" -eq 0 ]
    [[ "$output" == *"status=$s"* ]]
  done

  # Test complete separately — it moves to completed/ and clears pointer
  run bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" complete
  [ "$status" -eq 0 ]
  [[ "$output" == *"status=complete"* ]]
}

@test "set-status rejects invalid status" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "invalid-test")"
  run bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" "bogus"
  [ "$status" -eq 1 ]
  [[ "$output" == *"invalid status"* ]]
}

@test "set-status fails without active session" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" qa_pending
  [ "$status" -eq 1 ]
  [[ "$output" == *"no active"* ]]
}

@test "set-status updates the updated timestamp" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "ts-test")"
  local before_ts
  before_ts=$(grep '^updated:' "$session_file" | head -1)

  sleep 1
  bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" fix_applied > /dev/null
...
@test "increment-qa bumps round counter" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "qa-inc")"

  run bash "$SCRIPTS_DIR/debug-session-state.sh" increment-qa "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
  [[ "$output" == *"qa_round=1"* ]]
...
@test "increment-uat bumps round counter" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "uat-inc")"

  run bash "$SCRIPTS_DIR/debug-session-state.sh" increment-uat "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
  [[ "$output" == *"uat_round=1"* ]]
...
@test "increment-qa resets qa_last_result to pending" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "reset-test")"

  bash "$SCRIPTS_DIR/debug-session-state.sh" increment-qa "$LBWC_PLANNING_DIR" > /dev/null
...
@test "clear-active removes the pointer" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "clear-test")"
  [ -f "$LBWC_PLANNING_DIR/debugging/.active-session" ]

  run bash "$SCRIPTS_DIR/debug-session-state.sh" clear-active "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
  [ ! -f "$LBWC_PLANNING_DIR/debugging/.active-session" ]
}
...
@test "list shows no sessions when directory is empty" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" list "$LBWC_PLANNING_DIR"
  [ "$status" -eq 0 ]
  [[ "$output" == *"no_sessions=true"* ]]
}

@test "list shows sessions with status and location" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "list-test")"
...
@test "set-status complete moves session to completed directory" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "complete-move")"
  local fname
  fname=$(basename "$session_file")

  # Session starts in active/
  [ -f "$LBWC_PLANNING_DIR/debugging/active/$fname" ]

  bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" complete > /dev/null

  # Session moved to completed/
  [ ! -f "$LBWC_PLANNING_DIR/debugging/active/$fname" ]
  [ -f "$LBWC_PLANNING_DIR/debugging/completed/$fname" ]
  grep -q '^status: complete$' "$LBWC_PLANNING_DIR/debugging/completed/$fname"
  grep -q '^qa_last_result: skipped_no_fix_required$' "$LBWC_PLANNING_DIR/debugging/completed/$fname"
  grep -q '^uat_last_result: skipped_no_fix_required$' "$LBWC_PLANNING_DIR/debugging/completed/$fname"
}

@test "set-status complete preserves existing verified results" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "complete-preserve")"
  local fname
  fname=$(basename "$session_file")
  ...
  [ -f "$LBWC_PLANNING_DIR/debugging/completed/$fname" ]
  grep -q '^qa_round: 1$' "$LBWC_PLANNING_DIR/debugging/completed/$fname"
  grep -q '^qa_last_result: pass$' "$LBWC_PLANNING_DIR/debugging/completed/$fname"
  grep -q '^uat_round: 1$' "$LBWC_PLANNING_DIR/debugging/completed/$fname"
  grep -q '^uat_last_result: pass$' "$LBWC_PLANNING_DIR/debugging/completed/$fname"
}

@test "set-status complete clears active pointer" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "pointer-clear")"
  [ -f "$LBWC_PLANNING_DIR/debugging/.active-session" ]
...
@test "legacy flat-path session migrated on get-or-latest" {
  ...
  mkdir -p "$LBWC_PLANNING_DIR/debugging"
  local session_id="20250101-120000-legacy-bug"
  local legacy_file="$LBWC_PLANNING_DIR/debugging/${session_id}.md"
  ...
  echo "${session_id}.md" > "$LBWC_PLANNING_DIR/debugging/.active-session"
  ...
  [ ! -f "$legacy_file" ]
  [ -f "$LBWC_PLANNING_DIR/debugging/active/${session_id}.md" ]
}

@test "get-or-latest discovers unmigrated legacy session on migration collision" {
  mkdir -p "$LBWC_PLANNING_DIR/debugging/active"
  ...
  printf '%s\n' '---' "session_id: ${session_id}" 'title: collision-existing' 'status: complete' '---' > "$LBWC_PLANNING_DIR/debugging/active/${session_id}.md"
  ...
  [ -f "$LBWC_PLANNING_DIR/debugging/active/${session_id}.md" ]
  [ -f "$LBWC_PLANNING_DIR/debugging/completed/${session_id}.md" ]
...
@test "get-or-latest discovers legacy session via fallback when collision is non-complete" {
  ...
  ln -s /dev/null "$LBWC_PLANNING_DIR/debugging/active/${session_id}.md"

  # Capture stderr for warning assertion
  local stderr_file="$LBWC_PLANNING_DIR/stderr.tmp"
  run bash -c "bash '$SCRIPTS_DIR/debug-session-state.sh' get-or-latest '$LBWC_PLANNING_DIR' 2>'$stderr_file'"
  [ "$status" -eq 0 ]
...
@test "legacy complete session migrated to completed on list" {
  ...
  [ ! -f "$legacy_file" ]
  [ -f "$LBWC_PLANNING_DIR/debugging/completed/${session_id}.md" ]
  grep -q '^qa_last_result: pass$' "$LBWC_PLANNING_DIR/debugging/completed/${session_id}.md"
  grep -q '^uat_last_result: pass$' "$LBWC_PLANNING_DIR/debugging/completed/${session_id}.md"
}
...
@test "list emits warning to stderr when legacy migration fails" {
  ...
  printf '%s\n' '---' "session_id: ${session_id}" 'title: active-blocker' 'status: investigating' '---' > "$LBWC_PLANNING_DIR/debugging/active/${session_id}.md"

  # Capture stderr for warning assertion
  local stderr_file="$LBWC_PLANNING_DIR/stderr-list.tmp"
  run bash -c "bash '$SCRIPTS_DIR/debug-session-state.sh' list '$LBWC_PLANNING_DIR' 2>'$stderr_file'"
  [ "$status" -eq 0 ]
  [[ "$(cat "$stderr_file")" == *"Warning: could not migrate legacy session"* ]]
}
...
@test "list shows sessions from both active and completed" {
  ...
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "active-bug")"
  local active_file="$session_file"

  # Create another session and complete it
  sleep 1
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "done-bug")"
  bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" complete > /dev/null

  # Resume the first session to make it active again
  local active_name
  active_name=$(basename "$active_file")
  bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "$active_name" > /dev/null
...
@test "resume returns completed session metadata without reactivating it" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "resume-done")"
  local fname
  fname=$(basename "$session_file")
  ...
  run bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "$fname"
  [ "$status" -eq 0 ]
  [[ "$output" == *"active_session=true"* ]]
...
@test "resume migrates legacy completed session without reactivating it" {
  ...
  cat > "$LBWC_PLANNING_DIR/debugging/$session_name" << 'EOF'
---
...
EOF
  ...
  run bash "$SCRIPTS_DIR/debug-session-state.sh" resume "$LBWC_PLANNING_DIR" "$session_name"
  [ "$status" -eq 0 ]
  [[ "$output" == *"active_session=true"* ]]
  printf '%s\n' "$output" | grep -Eq '^session_status=complete$'
  [ ! -f "$LBWC_PLANNING_DIR/debugging/$session_name" ]
  [ ! -f "$LBWC_PLANNING_DIR/debugging/active/$session_name" ]
  [ -f "$LBWC_PLANNING_DIR/debugging/completed/$session_name" ]
  [ ! -f "$LBWC_PLANNING_DIR/debugging/.active-session" ]
}

@test "resume canonicalizes completed session stranded in active without reactivating it" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "resume-stranded-done")"
  local fname
  fname=$(basename "$session_file")
...
@test "get-or-latest recovers pointer to unresolved session stranded in completed directory" {
  ...
  mkdir -p "$LBWC_PLANNING_DIR/debugging/completed"
  cat > "$LBWC_PLANNING_DIR/debugging/completed/$session_name" << 'EOF'
---
...
# Stranded session
EOF
  echo "$session_name" > "$LBWC_PLANNING_DIR/debugging/.active-session"
  ...
  [ -f "$LBWC_PLANNING_DIR/debugging/active/$session_name" ]
  [ ! -f "$LBWC_PLANNING_DIR/debugging/completed/$session_name" ]
  [ "$(cat "$LBWC_PLANNING_DIR/debugging/.active-session")" = "$session_name" ]
}
...
@test "list self-heals complete sessions stranded in active directory" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "stranded-done")"
  local fname
  fname=$(basename "$session_file")
...
@test "get returns none after self-heal of completed session in active" {
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "selfheal-get")"
  local fname
  fname=$(basename "$session_file")
...
@test "get-or-latest falls back after self-heal of completed session" {
  ...
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "fallback-a")"
  ...
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "fallback-b")"
  local fname_b
  fname_b=$(basename "$session_file")

  # Set pointer back to A, then make A complete
  echo "$fname_a" > "$LBWC_PLANNING_DIR/debugging/.active-session"
  awk 'BEGIN{in_fm=0} /^---$/{in_fm=!in_fm} in_fm && /^status:/{print "status: complete";next} {print}' \
  ...
  [[ "$output" == *"active_session=fallback"* ]]
  [[ "$output" == *"fallback-b"* ]]
  grep -q '^qa_last_result: skipped_no_fix_required$' "$LBWC_PLANNING_DIR/debugging/completed/$fname_a"
  grep -q '^uat_last_result: skipped_no_fix_required$' "$LBWC_PLANNING_DIR/debugging/completed/$fname_a"
}
...
EOF
  ...
  [ -f "$LBWC_PLANNING_DIR/debugging/completed/$session_name" ]
  [ ! -f "$LBWC_PLANNING_DIR/debugging/$session_name" ]
  grep -q '^qa_last_result: skipped_no_fix_required$' "$LBWC_PLANNING_DIR/debugging/completed/$session_name"
  grep -q '^uat_last_result: skipped_no_fix_required$' "$LBWC_PLANNING_DIR/debugging/completed/$session_name"
}
...
EOF
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"active_session=none"* ]]
  grep -q '^qa_round: 1$' "$LBWC_PLANNING_DIR/debugging/completed/$session_name"
  grep -q '^qa_last_result: pass$' "$LBWC_PLANNING_DIR/debugging/completed/$session_name"
  grep -q '^uat_round: 1$' "$LBWC_PLANNING_DIR/debugging/completed/$session_name"
  grep -q '^uat_last_result: pass$' "$LBWC_PLANNING_DIR/debugging/completed/$session_name"
}

@test "get returns none for stale pointer to completed session" {
  # Create a session via normal flow, then complete it
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "stale-ptr")"
  local fname
  fname=$(basename "$session_file")
  bash "$SCRIPTS_DIR/debug-session-state.sh" set-status "$LBWC_PLANNING_DIR" complete > /dev/null
  [ -f "$LBWC_PLANNING_DIR/debugging/completed/$fname" ]
  ...
  echo "$fname" > "$LBWC_PLANNING_DIR/debugging/.active-session"
...
@test "list does not double-count self-healed sessions" {
  ...
  eval "$(bash "$SCRIPTS_DIR/debug-session-state.sh" start "$LBWC_PLANNING_DIR" "double-a")"
  local fname_a
  fname_a=$(basename "$session_file")
  awk 'BEGIN{in_fm=0} /^---$/{in_fm=!in_fm} in_fm && /^status:/{print "status: complete";next} {print}' \
    "$session_file" > "${session_file}.tmp" && mv "${session_file}.tmp" "$session_file"
  ...
  cat > "$LBWC_PLANNING_DIR/debugging/completed/$session_name_b" << 'EOF'
---
...
@test "unknown command fails with error" {
  run bash "$SCRIPTS_DIR/debug-session-state.sh" bogus "$LBWC_PLANNING_DIR"
  [ "$status" -eq 1 ]
