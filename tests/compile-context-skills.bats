...
@test "compile-context: no Available Skills section for any role" {
  ...
  for role in lead dev qa scout debugger architect; do
    cd "$TEST_TEMP_DIR"
    run bash "$SCRIPTS_DIR/compile-context.sh" 01 "$role" .lbwc-planning/phases
    [ "$status" -eq 0 ]
    ! grep -q "### Available Skills" "$TEST_TEMP_DIR/.lbwc-planning/phases/01-test-phase/.context-${role}.md"
  done
}
...
@test "compile-context: no available_skills XML tags in output" {
  ...
  cd "$TEST_TEMP_DIR"
  run bash "$SCRIPTS_DIR/compile-context.sh" 01 dev .lbwc-planning/phases
  [ "$status" -eq 0 ]
  ! grep -q "<available_skills>" "$TEST_TEMP_DIR/.lbwc-planning/phases/01-test-phase/.context-dev.md"
  ! grep -q "</available_skills>" "$TEST_TEMP_DIR/.lbwc-planning/phases/01-test-phase/.context-dev.md"
}
...
@test "compile-context: no mandatory skill activation (emit_skills_section removed)" {
  ...
  run bash "$SCRIPTS_DIR/compile-context.sh" 01 dev .lbwc-planning/phases
  [ "$status" -eq 0 ]
  ! grep -q "### Mandatory Skill Activation" "$TEST_TEMP_DIR/.lbwc-planning/phases/01-test-phase/.context-dev.md"
}
...
EOF
  ...
  HOME="$TEST_TEMP_DIR" run bash "$SCRIPTS_DIR/compile-context.sh" 01 dev .lbwc-planning/phases
  [ "$status" -eq 0 ]
  ! grep -q "### Mandatory Skill Activation" "$TEST_TEMP_DIR/.lbwc-planning/phases/01-test-phase/.context-dev.md"
}
