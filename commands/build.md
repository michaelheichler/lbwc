...
```bash
CONTRACT_PATH=$(bash "${CLAUDE_PLUGIN_ROOT}/scripts/task-contract.sh" open "$PLAN_PATH" "$PROJECT_ROOT" "$TASK_NAME" --role "$ROLE" --team "$TEAM_MODE" --group "$GROUP_NAME" --job "$BRIEF" "${CONTRACT_ALLOWANCE_ARGS[@]}")
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/lib/session-telemetry.py" record --event command --outcome success --phase "$PHASE"
```
