...
record_from_uat() {
  ...
  tmp_records=$(mktemp "${TMPDIR:-/tmp}/lbwc-accepted-deviations.XXXXXX")
  tmp_registry=$(mktemp "${TMPDIR:-/tmp}/lbwc-accepted-registry.XXXXXX")
...
planning_root_for_phase() {
  ...
  while [ "$dir" != "/" ] && [ -n "$dir" ]; do
    if [ "$(basename "$dir")" = ".lbwc-planning" ]; then
      printf '%s\n' "$dir"
      return 0
    fi
    dir=$(dirname "$dir")
